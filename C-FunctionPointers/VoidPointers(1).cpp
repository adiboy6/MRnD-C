...
void task7() {
	...
	void *a = getFloatAsVoidStar();
	printf("%f", *(float *)a);
	void *b = getIntAsVoidStar();
	printf("%d", *(int *)b);
	void *c = getStringAsVoidStar();
	printf("%f", *(char *)c);
}
...
void ** getTwoGreater(void *addr1, void *addr2, void *addr3, int(*compare)(void *, void *)) {
	//Use malloc and create an array of 2 void *.
	...
	if (compare(addr1, addr2) == 1 && compare(addr1, addr3) == 1)
		result[0] = addr1;
	else if (compare(addr2, addr1) == 1 && compare(addr2, addr3) == 1)
		result[0] = addr2;
	else
		result[0] = addr3;
	if (result[0] == addr1)
...
void task9() {
	...
	float b = 6.6;
	float c = 5.6;
	void **res=getTwoGreater((void *)&a, (void *)&b, (void *)&c, &compareFloat);
	printf("%f\n", *(float *)res[0]);
	printf("%f\n\n", *(float *)res[1]);
	...
	int e = 6.6;
	int f = 5.6;
	void **res1 = getTwoGreater((void *)&d, (void *)&e, (void *)&f, &compareInt);
	printf("%d\n", *(int *)res1[0]);
	printf("%d\n\n", *(int *)res1[1]);
	...
	char h = 'D';
	char i = 'I';
	void **res2 = getTwoGreater((void *)&g, (void *)&h, (void *)&i, &compareChar);
	printf("%c\n", *(char *)res2[0]);
	printf("%c\n\n", *(char *)res2[1]);
