...
void maximum(student *obj, int n, int *max )
{
	if (obj->first != NULL)
	{
		if (obj->second != NULL)
		{
			if (obj->third != NULL)
			{
				if (obj->letter - obj->third->letter != 0)
					maximum(obj->third, n + 1, max);
			}
			if (obj->letter - obj->second->letter != 0)
				maximum(obj->second, n + 1, max);
		}
		if (obj->letter - obj->first->letter != 0)
			maximum(obj->first, n + 1, max);
	}
	if (n > *max)
		*max = n;
}

void minimum(student *obj, int m, int *min)
{
	if (obj->first == NULL || obj->second == NULL || obj->third == NULL)
	{
		if (m < *min)
			*min = m;
	}
	...
	{
		if (obj->letter - obj->first->letter != 0)
			minimum(obj->first, m + 1, min);

		if (obj->letter - obj->second->letter != 0)
			minimum(obj->second, m + 1, min);

		if (obj->letter - obj->third->letter != 0)
			minimum(obj->third, m + 1, min);
	}*/
	if (obj->first != NULL)
	{
		if (obj->second != NULL)
...
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance)
{
	if (leaderStudent == NULL)
		return;
	*minBallDistance = 100000;
	//int m=1;
	minimum(leaderStudent, 1, minBallDistance);
	maximum(leaderStudent, 1, maxBallDistance);
	return;
}
