...
void shortestpathf(node*root, int k, int *shortestpath, int *shortestpathlen, int *sp, int spl, int sum, int *ways)
{
	if (root == NULL)
	{
		return;
	...
	sp[spl - 1] = root->data;
	sum = sum + root->data;
	shortestpathf(root->left, k, shortestpath, shortestpathlen, sp, spl + 1, sum, ways);
	shortestpathf(root->right, k, shortestpath, shortestpathlen, sp, spl + 1, sum, ways);
	if (sum == k&&root->left == root->right){
		if (spl < *shortestpathlen)
		{
			int i;
			for (i = 0; i < spl; i++)
			{
...
int mangocity_count_ways(struct node *startcity, int k, int *shortestpath, int *shortestpathlen) {
	...
	*shortestpathlen = 1001;
	int arr[1000];
	int ways = 0;
	shortestpathf(startcity, k, shortestpath, shortestpathlen, arr, 1, 0, &ways);
	if (ways == 0)
		return -1;
	return ways;
}