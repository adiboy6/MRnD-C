...
void shortpath(struct node *curr, int k, int *path, int *len, int *sp, int spl, int s, int *ways)
{
	if (curr != NULL)
	{
		sp[spl - 1] = curr->data;
		s = s + curr->data;
		shortpath(curr->left, k, path, len, sp, spl + 1, s, ways);
		shortpath(curr->right, k, path, len, sp, spl + 1, s, ways);
		if (s == k && curr->left == curr->right)
		{
			if (spl < *len)
			{
				for (int i = 0; i < spl; i++)
...
int mangocity_count_ways(struct node *startcity, int k, int *shortestpath, int *shortestpathlen) {
	...
	*shortestpathlen = 1001;
	int arr[1000];
	int ways = 0;
	shortpath(startcity, k, shortestpath, shortestpathlen, arr, 1, 0, &ways);
	if (ways == 0)
		return -1;
	return ways;
}